...
fn main() -> anyhow::Result<()> {
    let args = Args::parse();
    println!("Worker Node starting, connecting to shmem-id: {}", args.shmem_id);

    let shmem = ShmemConf::new().os_id(&args.shmem_id).open().context("Failed to open shared memory")?;
